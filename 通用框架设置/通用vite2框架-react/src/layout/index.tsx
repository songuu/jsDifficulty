...
import {setShowSidebar, getSelectRoleList} from '../store/actions'
...
const BaseLayout: React.FC<IBaseLayoutProps> = () => {
  ...
  /** 获取系统路由，菜单 */
  useEffect(() => {
    dispatch<any>(getSelectRoleList())
    findRoutes<ResponseData>().then((res: any) => {
      const {data} = res
      if (data.code === HttpStatusCode.OK) {
        const list = data.data

        const treeMenus: any = treeDataTranslate(list, 'id', 'parentId', {
          key: 'id',
          value: 'id',
          title: 'name',
        })
