/*
 * @Author: songyu
 * @Date: 2021-08-25 21:21:11
 * @LastEditTime: 2021-08-25 21:21:11
 * @LastEditors: songyu
 * @Description:
 * @FilePath: \项目文件\jsDifficulty\手写\最小二叉堆堆的实现.js
 */
"use strict";
...
module.exports = class PriorityQueue {
  constructor(comparator, setPosition) {
    if (comparator !== undefined) this[kCompare] = comparator;
    if (setPosition !== undefined) this[kSetPosition] = setPosition;
  ...
  insert(value) {
    ...
    if (heap.length === pos) heap.length *= 2;
  ...
  percolateDown(pos) {
    ...
    while (pos * 2 <= size) {
      ...
      if (setPosition !== undefined) setPosition(child, pos);
      heap[pos] = child;
      pos = childIndex;
    }
    heap[pos] = item;
    if (setPosition !== undefined) setPosition(item, pos);
  }
  ...
  percolateUp(pos) {
    ...
    while (pos > 1) {
      ...
      heap[pos] = parent;
      if (setPosition !== undefined) setPosition(parent, pos);
      pos = (pos / 2) | 0;
    }
    heap[pos] = item;
    if (setPosition !== undefined) setPosition(item, pos);
